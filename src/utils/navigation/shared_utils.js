...
var BACK_KEY_NAME = ["Backspace", "Escape"];
...
const SharedUtils = {
    ...
    readColorCode: function su_readColorCode(blob, x, y, callback) {
        ...
        offscreenImg.onload = function () {
            ...
            try {
                ...
                ctx.drawImage(
                    offscreenImg,
                    offscreenImg.naturalWidth * x,
                    offscreenImg.naturalHeight * y,
                    1,
                    1,
                    0,
                    0,
    ...
    localizeElement: function su_localizeElement(element, payload) {
        ...
        if (typeof payload === "object") {
            if (payload.id) {
                document.l10n.setAttributes(element, payload.id, payload.args);
                return;
            } else if (payload.raw) {
    ...
    isBackKey: function su_isBackKey(event) {
        return (
            BACK_KEY_CODE.includes(event.keyCode) ||
    ...
    createSmartDialog: function su_createSmartDialog(type, container, options) {
        ...
        switch (type) {
            case "modal":
                Constructor =
                    window.FxosTvModalDialog || window.SmartModalDialog;
                break;
            case "input":
                Constructor =
                    window.FxosTvInputDialog || window.SmartInputDialog;
                break;
            default:
                return null;
        }
        ...
        dialogElement.addEventListener("keyup", (evt) => {
            if (
                this.isBackKey(evt) &&
                dialogElement.classList.contains("opened")
            ) {
                evt.preventDefault();
                evt.stopPropagation();
                dialogElement.close();
