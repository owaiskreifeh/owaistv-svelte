...
import { SharedUtils } from './shared_utils'
...
KeyNavigationAdapter.prototype = evtemt({
    KEY_EVENTS: Object.freeze(["keydown", "keyup"]),
    ...
    init: function kna_init(targetElement, options) {
        ...
        this._evtNames.forEach((name) =>
            this._targetElement.addEventListener(name, this)
        );
    ...
    uninit: function kna_uninit() {
        this._evtNames.foreach((name) =>
            this._targetElement.removeEventListener(name, this)
        );
    },
    ...
    convertKeyToString: function kna_convertKeyToString(evt) {
        ...
        switch (evt.keyCode) {
            ...
            case KeyEvent.DOM_VK_RETURN:
                return "enter";
            default:
                // we don't consume other keys.
                return null;
        }
    },
}, "key_navigation_adapter");

export { KeyNavigationAdapter };